...
static long _get_data(OggVorbis_File *vf){
  errno=0;
  if(vf->datasource >= 0){//很重要的入口?,第几次进来,buffer地址是不是 0X20009EA8??
...
static int _bisect_forward_serialno(OggVorbis_File *vf,
  ...
  if(searched>=end || ret<0){
    ...
    vf->serialnos=(ogg_uint32_t    *)_ogg_malloc(vf->links*sizeof(*vf->serialnos));
...
static int _ov_open1(void *f,OggVorbis_File *vf,char *initial,
  ...
  int offsettest=0;// (f?callbacks.seek_func(f,0,SEEK_CUR):-1);
  int ret;
...
int ov_clear(OggVorbis_File *vf){
  if(vf){
    ...
    if(vf->datasource >= 0)(vf->callbacks.close_func)(vf->datasource);
    memset(vf,0,sizeof(*vf));
  }
#ifdef DEBUG_LEAKS
  _VDBG_dump();
...
size_t fread_handler(void *ptr, size_t size, size_t nmemb, void *datasource)
{
	u32 br;
	f_read((FIL*)datasource, ptr, size*nmemb,&br) ;
	return br;

}

int fclose_handler(void *datasource)
{
	f_close((FIL*)datasource);
	return 0;  
}

long ftell_handler(void *datasource)
{
	FIL*p;
	p=(FIL*)datasource; 
	return p->fptr;  
...
static int _fseek64_wrap(void* f,ogg_int64_t off,int whence)
{
	...
	switch(whence)
	{
		case SEEK_SET:	//指向文件头,然后偏移off位置
			f_lseek(p,off);
			break;
		case SEEK_CUR:	//指向文件当前位置,然后偏移off位置
			f_lseek(p,p->fptr+off);
			break;
		case SEEK_END:	//指向文件尾,然后偏移off位置
			f_lseek(p,p->fsize+off);
			break;						
	}
	return 0;
}
...
ov_callbacks callbacks = {
    (size_t (*)(void *, size_t, size_t, void *))  fread_handler,
    (int (*)(void *, ogg_int64_t, int))           _fseek64_wrap,
    (int (*)(void *))                             fclose_handler,
    (long (*)(void *))                            ftell_handler
};


int ov_open(void* f,OggVorbis_File *vf,char *initial,long ibytes){

...
static void *_insert(void *ptr,long bytes,char *file,long line){
  ...
  return((unsigned char*)ptr+HEAD_ALIGN);
}
...
static void _ripremove(void *ptr){
  ...
  if(pointers[insert]==NULL){
    printf("DEBUGGING MALLOC ERROR: freeing previously freed memory\n");
    printf("\t%s %ld\n",((head *)ptr)->file,((head *)ptr)->line);
  }

  if(global_bytes<0){
    printf("DEBUGGING MALLOC ERROR: freeing unmalloced memory\n");
  }
...
void _VDBG_dump(void){
  ...
  for(i=0;i<ptop;i++){
    head *ptr=pointers[i];
    if(ptr)
      printf("unfreed bytes from %s:%ld\n",
	      ptr->file,ptr->line);
  }

...
int main(void){
  ...
  for(i=0;i<test2size;i++){
    ...
    if(test!=large[i]){
      printf("%ld != %ld (%lx!=%lx):",test,large[i],
              test,large[i]);
      report("read incorrect value!\n");
    }
    oggpack_adv(&r,32);
  ...
  printf("\nTesting read past end (LSb): ");
  {
    ...
    for(i=0;i<64;i++){
      if(oggpack_read(&r,1)<0){
        printf("failed; got -1 prematurely.\n");
        exit(1);
      }
    }
    if(oggpack_look(&r,1)!=-1 ||
       oggpack_read(&r,1)!=-1){
      printf("failed; read past end without -1.\n");
      exit(1);
    }
  }
  {
    ...
    if((test=oggpack_read(&r,30))==0xffffffffUL || 
      ...
      printf("failed 2; got -1 prematurely.\n");
      exit(1);
    }
    
    if((test=oggpack_look(&r,18))==0xffffffffUL){
      printf("failed 3; got -1 prematurely.\n");
      exit(1);
    }
  ...
  for(i=0;i<10000;i++){
    ...
    printf("\rRandomized testing (LSb)... (%ld)   ",10000-i);
...
,
  ...
  printf("\rRandomized testing (LSb)... ok.   \n");

...
uint8_t*const AUDIO_BTN_PIC_TBL[2][5]=  	//5个图片按钮的路径
{
    {
...
uint8_t*const AUDIO_BACK_PIC[3]=										//背景图片
{
...
void audio_play_task(void *pdata)
{
    ...
    while(audiodev.status&0x80)
    {
        ...
        while(rval==0)
        {
            ...
            if(rval)break;//打开失败
            audiodev.name=(uint8_t*)(*audioinfo.lfname?audioinfo.lfname:audioinfo.fname);
...
uint8_t audio_filelist(__audiodev *audiodevx)
{
    ...
    else gui_memset((uint8_t *)audioinfo.lfname,0,audioinfo.lfsize);
    ...
    while(rval==0)
    {
        ...
        {
            /*
            size of audiodevx->path:    9
            audiodevx->path:0:\MUSIC
            size of audiodevx->mfindextbl:   30
            audiodevx->mfindextbl[0]:0x00000000
            audiodevx->mfindextbl[1]:0x00000005
            audiodevx->mfindextbl[2]:0x00000006
            audiodevx->mfindextbl[3]:0x0000000A
            audiodevx->mfindextbl[4]:0x0000000C
            audiodevx->mfindextbl[5]:0x0000000E
            audiodevx->mfindextbl[6]:0x00000011
            audiodevx->mfindextbl[7]:0x00000013
            audiodevx->mfindextbl[8]:0x00000017
            audiodevx->mfindextbl[9]:0x0000001B
            audiodevx->mfindextbl[10]:0x0000001D
            audiodevx->mfindextbl[11]:0x0000001F
            audiodevx->mfindextbl[12]:0x00000021
            audiodevx->mfindextbl[13]:0x00000023
            audiodevx->mfindextbl[14]:0x00000028
            size of audiodevx->mfilenum:   15
            play:0:\MUSIC\seve.mp3

            size of audiodevx->path:    9
            audiodevx->path:1:\MUSIC
            size of audiodevx->mfindextbl:    2
            audiodevx->mfindextbl[0]:0x00000000
            size of audiodevx->mfilenum:    1
            play:1:\MUSIC\侧脸-于果.mp3
            audiombox :    1
            ...
            */
            ...
            printf("size of audiodevx->path:%5d\r\n",strlen((const char*)flistbox->path)+1);
            ...
            printf("audiodevx->path:%s\r\n",(const char *)audiodevx->path);
            audiodevx->mfindextbl=(uint16_t*)gui_memex_malloc(flistbox->filecnt*2);//为新的tbl申请内存
            printf("size of audiodevx->mfindextbl:%5d\r\n",flistbox->filecnt*2);
            ...
            {
                audiodevx->mfindextbl[i]=flistbox->findextbl[i];//复制
                printf("audiodevx->mfindextbl[%d]:0x%08X\r\n",i,(int)audiodevx->mfindextbl[i]);
            }
            audiodevx->mfilenum=flistbox->filecnt;		//记录文件个数
            printf("size of audiodevx->mfilenum:%5d\r\n",flistbox->filecnt);
            OSMboxPost(audiombox,(void*)(flistbox->selindex-flistbox->foldercnt+1));//发送邮箱,因为邮箱不能为空,所以在这必须加1
            printf("audiombox :%5d\r\n",flistbox->selindex-flistbox->foldercnt+1);
            flistbox->dbclick=0;
            break;
        }
    }

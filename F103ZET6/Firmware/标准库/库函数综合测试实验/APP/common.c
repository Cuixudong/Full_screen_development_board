...
//多行提醒
//x,y:起始坐标
//widht,height:宽度和高度,宽度必须大于100,高度必须大于90
//建议widht:等于字符个数*6+20,且width>100
//建议height:等于条目数*16+90
//caption:标题
//str:字符串(条目之间以回车符:'\r'区分,条目高度固定为16)
void app_muti_remind_msg(u16 x,u16 y,u16 width,u16 height,u8*caption,u8 *str)
{
    ...
    if(twin&&qbtn&&tbuf)
    {
        ...
        qbtn->caption=(u8*)GUI_OK_CAPTION_TBL[gui_phy.language];//标题为确定
...
u8*const lwip_webserver_path_tbl[19]=
{
    "1:/SYSTEM/LWIP/WebServer/404.html",
    "1:/SYSTEM/LWIP/WebServer/index.html",
    "1:/SYSTEM/LWIP/WebServer/openedv.html",
    ...
    "1:/SYSTEM/LWIP/WebServer/openedv/folder_big.gif",
    "1:/SYSTEM/LWIP/WebServer/openedv/folder_new_big.gif",
    "1:/SYSTEM/LWIP/WebServer/openedv/h.js",
    "1:/SYSTEM/LWIP/WebServer/openedv/head_weixin1.png",
    "1:/SYSTEM/LWIP/WebServer/openedv/logo.jpg",
    "1:/SYSTEM/LWIP/WebServer/openedv/newpm.gif",
    "1:/SYSTEM/LWIP/WebServer/openedv/wb.js",
};
//将磁盘路径按要求修改
//des:目标缓存区
//diskx:新的磁盘编号
...
//		if(j!=3)break;
        ...
        if(j!=3&&i!=16)break;
        //APP/PAINT 文件夹内1个图标检测
        app_system_file_chgdisk(path,diskx,PAINT_COLOR_TBL_PIC);
        res=f_open(f_check,(const TCHAR*)path,FA_READ);//只读方式打开文件
        if(res)break;			//打开失败
        f_close(f_check);		//关闭文件
        rval++;
        //APP/TOMCAT 文件夹内1个图标检测
        app_system_file_chgdisk(path,diskx,TOMCAT_DEMO_PIC);
        res=f_open(f_check,(const TCHAR*)path,FA_READ);//只读方式打开文件
        if(res)break;			//打开失败
        f_close(f_check);		//关闭文件
        rval++;
...
u8*const APP_SYSTEM_APPPATH="/SYSTEM/APP";	//APP文件夹路径
...
u8 app_system_parameter_init(void)
{
    ...
    if(systemset.saveflag!=0X0A||systemset.syslanguage>2)//之前没有保存过/语言信息错误
    {
        ...
        systemset.lcdbklight=0;			//背光默认为自动背光控制
...
// 	LCD_PWM_Init();						//初始化LCD背光 PWM输出,作为LCD的背光控制
...
void app_set_default(void)
{
    //系统设置参数,恢复默认设置
    systemset.syslanguage=0;		//默认为简体中文
    systemset.lcdbklight=0;			//背光默认为自动背光
    systemset.picmode=0;			//图片顺序播放
    systemset.audiomode=0;			//音乐播放默认是按顺序播放
    systemset.videomode=0;			//视频播放默认是按顺序播放
    systemset.saveflag=0X0A;  		//设置保存标记

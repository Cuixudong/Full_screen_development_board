...
void net_load_ui(void)
{
    ...
    if(temp>1)temp=10;	//10M网速
    else temp=100;		//100M网速
...
void net_tcpserver_remove_timewait(void)
{
    ...
    u8 t=0;
    while(tcp_active_pcbs!=NULL)				//等待tcp_active_pcbs为空
    {
        t++;
        delay_ms(10);
        if(t>100)break;		//最多等待10秒钟
    }
    pcb=tcp_tw_pcbs;
...
u8 net_test(void)
{
    ...
    p=gui_memex_malloc(1500);	//申请1500字节内存
...
u8 net_play(void)
{
    ...
    system_task_return=0;
    lwip_comm_destroy();
    DM9000_RST=0;			//保持DM9000复位状态,减少功耗.

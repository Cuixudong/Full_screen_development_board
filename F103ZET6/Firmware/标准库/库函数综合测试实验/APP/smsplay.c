    ...
    if(head->item)//item有数据
    {
        ...
        while(1)
        {
            ...
            if(res&0X80)
            {
                ...
                {
                    fsel=0;
                    if(msgnum)res=app_items_sel((lcddev.width-180)/2,(lcddev.height-192)/2,180,72+40*3,(u8**)sms_option_tbl[gui_phy.language],3,(u8*)&fsel,0X90,(u8*)sms_remind_msg_tbl[1][gui_phy.language]);//3个选择

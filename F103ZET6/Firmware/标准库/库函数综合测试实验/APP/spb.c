...
u8 spb_init(void)
{
    ...
    if(lcdtype==2)
    {
        sramlcdbuf=gui_memex_malloc(lcddev.width*(lcddev.height-spbdev.stabarheight)*2);	//1帧缓存就够了
...
//	}
//	else
    gui_show_string(spb_sim900a_msg[gui_phy.language][0],2+20+2,(spbdev.stabarheight-16)/2,64,16,16,WHITE);//显示运营商名字
    //更新SD卡信息
...
u8 spb_move_chk(void)
{
    ...
    tp_dev.scan(0);	//扫描
    if(lcddev.width==320)skips=15;	//3.5屏跳15个像素

...
//系统状提示信息
uint8_t*const sysset_sysstatus_tbl[4][GUI_LANGUAGE_NUM]=
{
...
{
    {"ALIENTEK战舰V3","ALIENTEK戰艦V3","ALIENTEK WarShip V3",},
    ...
    while(rval==0)
    {
        ...
        for(i=0; i<2; i++)
        {
            ...
            if(res&&(voltemp[i]!=vs1053prgb[i]->curpos))		//进度条改动了
            {
                ...
                VS_Set_Bass(vsxset->bflimit,vsxset->bass,vsxset->tflimit,vsxset->treble);
...
void sysset_system_update(uint8_t*caption,uint16_t sx,uint16_t sy)
{
    ...
    if(twin!=NULL)
    {
        ...
        cpymsg_x=sx;
        cpymsg_y=sy;
        res=app_system_update(sysset_system_update_cpymsg,"0:");		//从SD卡更新
        p=(uint8_t*)sysset_system_update_err_tbl[0][gui_phy.language];		//更新成功提示
        if(res==0XFF)p=(uint8_t*)sysset_system_update_err_tbl[2][gui_phy.language];	//强制退出
        else if(res)p=(uint8_t*)sysset_system_update_err_tbl[1][gui_phy.language];	//系统文件丢失
        window_msg_box(sx,sy+20,200,80,p,APP_REMIND_CAPTION_TBL[gui_phy.language],12,0,0,0);
        delay_ms(1500);
    }
    window_delete(twin);
...
void sysset_system_status(uint16_t x,uint16_t y,uint8_t* caption)
{
    ...
    twin=window_creat(x,y,200,206,0,1|1<<5,16);					//创建窗口
    cpuprgb=progressbar_creat(x+10,y+32+19,180,18,0X61);		//创建进度条
    meminprgb=progressbar_creat(x+10,y+32+19+35,180,18,0X61);	//创建进度条
    memexprgb=progressbar_creat(x+10,y+32+19+35*2,180,18,0X61);	//创建进度条
    rbtn=btn_creat(x+50,y+32+19+35*2+36,SYSSET_BTN3_WIDTH,SYSSET_BTN2_HEIGHT,0,0x02);//创建按钮
    ...
    else
    {
        ...
        gui_show_string((uint8_t*)sysset_sysstatus_tbl[2][gui_phy.language],x+10,y+32+5+35*2,190,12,12,SYSSET_INWIN_FONT_COLOR);	//显示外部内存使用率
        gui_show_string((uint8_t*)sysset_sysstatus_tbl[3][gui_phy.language],x+200-10-36-36,y+32+5,190,12,12,SYSSET_INWIN_FONT_COLOR);//显示温度
        ...
        if(res&0X80)
        {
            ...
            case 8://板载喇叭开关设置
                ...
                if(res==0)//设置成功
                {
                    ...
                    VS_SPK_Set(vsset.speakersw);	//设置喇叭状态
                    savemask|=1<<0;					//标记系统设置数据改动了
                }
                break;

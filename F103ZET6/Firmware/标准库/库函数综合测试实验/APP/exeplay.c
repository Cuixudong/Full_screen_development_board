...
void exeplay_write_appmask(u16 val)
{
    ...
    PWR->CR|=1<<8;      	//取消备份区写保护
    BKP->DR2=val;			//标识字,写入DR2
}
//在主函数最开始的时候被调用.
//检测是否有app程序需要执行.如果是,则直接执行.
void exeplay_app_check(void)
{
    u8 *xdes=(u8*)EXEPLAY_APP_BASE;		//目标地址
    u8 *xsrc=(u8*)EXEPLAY_SRC_BASE;		//源地址
    u32 n=EXEPLAY_APP_SIZE;				//数据长度
    if(BKP->DR2==0X5050)				//检查BKP2,如果为0X5050,则说明需要执行app代码
    {

...
uint8_t system_exsram_test(uint16_t x,uint16_t y,uint8_t fsize)
{
    ...
    for(i=0; i<1024*1024; i+=1024)
    {
        ...
        LCD_ShowxNum(x+15*(fsize/2),y,(uint16_t)(temp-sval+1),4,fsize,0);//显示内存容量
    }
    if(i>=1024*1024)
    {
...
extern uint8_t Max_Lun;	//支持的磁盘个数,0表示1个,1表示2个.
//USB更新SPI FLASH确认.
//x,y:坐标
//fsize:字体大小
//返回值:0,不需要更新;1,确认更新
uint8_t system_usb_update_confirm(uint16_t x,uint16_t y,uint8_t fsize)
{
    uint8_t key;
    uint8_t t=0;
    uint8_t res=0;
    uint8_t offline_cnt=0;
    uint8_t Divece_STA=0;
    uint8_t USB_STA=0;
    POINT_COLOR=RED;
    ...
    if(res)
    {
        ...
        Data_Buffer=(uint32_t*)gui_memin_malloc(BULK_MAX_PACKET_SIZE*2*4);	//为USB通讯申请内存
...
//显示版权信息
    ...
    if(lcddev.width==240)
    {
        ...
        icowidth=18;
        okoffset=190;
        app_show_mono_icos(5,ypos,icowidth,24,(uint8_t*)APP_ALIENTEK_ICO1824,YELLOW,BLACK);
    } else if(lcddev.width==320)
    {
        fsize=16;
        icowidth=24;
        okoffset=250;
        app_show_mono_icos(5,ypos,icowidth,32,(uint8_t*)APP_ALIENTEK_ICO2432,YELLOW,BLACK);
    } else if(lcddev.width==480)
    {
        fsize=24;
        icowidth=36;
        okoffset=370;
        app_show_mono_icos(5,ypos,icowidth,48,(uint8_t*)APP_ALIENTEK_ICO3648,YELLOW,BLACK);
    }
...
void main_task(void *pdata)
{
    ...
    while(1)
    {
        //bmp_encode("0:PICTURE/ScreenCut004.bmp",0,0,480,800,0)截图串口命令
        selx=spb_move_chk();
...
//		if(USART3_RX_STA & 0x08000)	//TPAD按下了一次,此函数执行,至少需要15ms.
//		{
//			printf("usart3 data:%s\r\n",USART3_RX_BUF);//打印内存占用率
//			USART3_RX_STA = 0;
//			if(strstr((const char *)USART3_RX_BUF,"TaskReturn"))
//			system_task_return=1;
//			if(gifdecoding)gifdecoding=0;	//不再播放gif
//		}

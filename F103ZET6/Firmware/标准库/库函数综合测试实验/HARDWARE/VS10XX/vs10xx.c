...
{
    SPI1_SetSpeed(SPI_BaudRatePrescaler_32 + (nes_spped_para<<3) );	//设置到低速模式
}
//SD卡正常工作的时候,可以高速了
void VS_SPI_SpeedHigh(void)
{
    SPI1_SetSpeed(SPI_BaudRatePrescaler_8 +(nes_spped_para<<3));	//设置到高速模式
}
//初始化VS10XX的IO口
void VS_Init(void)
{
    ...
    RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOE, ENABLE);	 //使能PC,PE,PF端口时钟

    GPIO_InitStructure.GPIO_Pin = GPIO_Pin_2;				 //PE2 VDS_DREQ
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU; 		 //输入
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
    GPIO_Init(GPIOE, &GPIO_InitStructure);

    GPIO_InitStructure.GPIO_Pin = GPIO_Pin_3;	 //PE3 VS_RST
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 //推挽输出
    GPIO_Init(GPIOE, &GPIO_InitStructure);

    GPIO_InitStructure.GPIO_Pin = GPIO_Pin_5|GPIO_Pin_4;//PE5,PE4 VS_DCS VS_CS
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 //推挽输出
    GPIO_Init(GPIOE, &GPIO_InitStructure);

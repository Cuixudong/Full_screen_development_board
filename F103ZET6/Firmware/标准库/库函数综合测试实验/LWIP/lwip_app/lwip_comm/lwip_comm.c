...
u8 lwip_comm_init(void)
{
	...
	struct ip_addr gw;      			//默认网关 
 	...
 	dm9000lock=OSMutexCreate(4,&err);	//创建互斥信号量,提高到优先级4	
	
	
	if(DM9000_Init(1))return 2;			//初始化DM9000AEP
...
void lwip_comm_destroy(void)
{
...
#endif 
	EXTI->EMR|=1<<6;				//禁止中断线6的中断
	DM9000_RST=0;					//复位DM9000
	OSTaskDel(LWIP_DM9000_INPUT_TASK_PRIO);//删除DM9000接收处理线程
	...
	ram_end=NULL;
	lfree=NULL; 
	OSSemDel(dm9000input,OS_DEL_ALWAYS,&err);	//删除信号量
	OSMutexDel(dm9000lock,OS_DEL_ALWAYS,&err);	//删除互斥信号量

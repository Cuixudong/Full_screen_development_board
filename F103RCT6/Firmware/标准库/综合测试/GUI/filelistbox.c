...
//磁盘名字表
uint8_t*const FLIST_DISK_NAME_TBL[3][GUI_LANGUAGE_NUM]=
{
	{"SD卡","SD卡","SD Card",},
...
volatile uint8_t FLIST_DISK_ORDER_TBL[3];
//filelist磁盘路径表
uint8_t*const FLIST_DISK_PATH_TBL[3]={"0:","1:","2:"};
...
uint8_t filelistbox_scan_filelist(_filelistbox_obj *filelistbox)
{
	...
	uint16_t typeex=0;
	uint16_t findex;	  
    uint8_t *fn;   //This function is assuming non-Unicode cfg. 
 	...
 	if(filelistbox->path)	//路径有效
	{
		...
		if(res==FR_OK)		//打开目录成功了.
		{
			...
			filelistbox->findextbl=(uint16_t*)gui_memin_malloc(FLBOX_MAX_DFFILE*2);//先默认申请FLBOX_MAX_DFFILE个目标文件索引内存
 		} 
		while(res==FR_OK)
		{
			findex=(uint16_t)flboxdir.index;//记录下此文件的index值
			...
			fn=(uint8_t*)(*flboxfinfo.lfname?flboxfinfo.lfname:flboxfinfo.fname);
		...
		if(mark==1&&filelistbox->filecnt)
		{
			...
			if(fn!=NULL)filelistbox->findextbl=(uint16_t*)fn;//如果申请成功了,则使用新的地址,否则源地址不变
		}   

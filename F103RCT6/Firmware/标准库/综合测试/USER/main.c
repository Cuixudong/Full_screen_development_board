...
//开始硬件检测初始化
    ...
    LED0=1;
    LED1=1;				//同时关闭两个LED
    if(SPI_Flash_ReadID()!=W25Q128)//检测不到W25Q128
    {
...
//强制要求任务返回
volatile uint8_t system_task_return=0;
//监视任务
void watch_task(void *pdata)
{
    OS_CPU_SR cpu_sr=0;
...
//				LCD_ShowNum(20,20,aacx,6,16);
//				LCD_ShowNum(20,40,aacy,6,16);
//				LCD_ShowNum(20,60,aacz,6,16);
                if((abs(aacx) < 6500) && (aacy > 12000))
                {
                    //左侧屏幕为上
                    g_lcd_dir = 2;
                }

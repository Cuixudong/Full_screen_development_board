...
uint8_t stdbmp_decode(const uint8_t *filename) 
{
	...
	databuf=(uint8_t*)mymalloc(readlen);		//开辟readlen字节的内存区域
...
{				
...
#if BMP_USE_MALLOC == 1	//使用malloc	
	databuf=(uint16_t*)mymalloc(1024);		//开辟至少bi4width大小的字节的内存区域 ,对240宽的屏,480个字节就够了.
	if(databuf==NULL)return PIC_MEM_ERR;		//内存申请失败.
	f_bmp=(FIL *)mymalloc(sizeof(FIL));	//开辟FIL字节的内存区域 
	if(f_bmp==NULL)								//内存申请失败.
	{		 
	...
	mymemset((uint8_t*)&hbmp,0,sizeof(hbmp));//置零空申请到的内存.	    

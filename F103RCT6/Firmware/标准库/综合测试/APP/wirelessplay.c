...
uint8_t wireless_play(void)
{
	...
	uint8_t tmp_buf[6];			//buf[0~3]:坐标值;buf[4]:0,正常画图;1,清屏;2,退出. 
	uint16_t x=0,y=0;
	...
	{
    	caption=(uint8_t*)APP_MFUNS_CAPTION_TBL[7][gui_phy.language];//无线传输	 
  	...
  	SPI1_SetSpeed(SPI_BaudRatePrescaler_4);//恢复SPI1速度为18Mhz   

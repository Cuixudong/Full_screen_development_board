...
const uint8_t APP_ALIENTEK_ICO1824[]=
{
...
//PCtoLCD2002取模方式:阴码,逐行式,顺向
const uint8_t APP_ALIENTEK_ICO2432[]=
{
...
//PCtoLCD2002取模方式:阴码,逐行式,顺向
const uint8_t APP_ALIENTEK_ICO3648[]=
{
...
uint8_t app_system_file_check(uint8_t* diskx)
{
	...
	{	
		//主界面图标检测,9个. 
 		for(i=0;i<9;i++)
		{
			app_system_file_chgdisk(path,diskx,(uint8_t*)mui_icos_path_tbl[i]); 
		...
		//检测应用程序16个图标
   		for(i=0;i<16;i++)
		{
			app_system_file_chgdisk(path,diskx,(uint8_t*)appplay_icospath_tbl[i]); 
	 		res=f_open(f_check,(const TCHAR*)path,FA_READ);//只读方式打开文件
			if(res)break;		//打开失败       
			f_close(f_check);    //关闭文件  
		}
...
uint8_t*const  APP_SYSTEM_APPPATH="0:/SYSTEM/APP";		//APP文件夹路径
uint8_t*const  APP_SYSTEM_FONTPATH="0:/SYSTEM/FONT";		//FONT文件夹路径
uint8_t*const  APP_SYSTEM_SYSICOPATH="0:/SYSTEM/SYSICO";	//SYSICO文件夹路径
...
uint8_t app_system_update(uint8_t(*fcpymsg)(uint8_t*pname,uint8_t pct,uint8_t mode))
{
	...
	uint32_t cpdsize=0;
	uint8_t res=0;
	totsize+=exf_fdsize((uint8_t*)APP_SYSTEM_APPPATH);//得到三个文件夹的总大小
	totsize+=exf_fdsize((uint8_t*)APP_SYSTEM_FONTPATH);
	totsize+=exf_fdsize((uint8_t*)APP_SYSTEM_SYSICOPATH);
	printf("totsize:%d\r\n",totsize);
	f_mkdir((const TCHAR *)APP_SYSTEM_DSTPATH);//强制创建目标文件夹
 	res=exf_fdcopy(fcpymsg,(uint8_t*)APP_SYSTEM_APPPATH,(uint8_t*)APP_SYSTEM_DSTPATH,&totsize,&cpdsize,1);
	if(res)
	{
		if(res!=0xff)return 1;	//第一阶段复制出错
		else return 0XFF;		//强制退出
	}
	res=exf_fdcopy(fcpymsg,(uint8_t*)APP_SYSTEM_FONTPATH,(uint8_t*)APP_SYSTEM_DSTPATH,&totsize,&cpdsize,1);
	if(res)
	{
		if(res!=0xff)return 1;	//第二阶段复制出错
		else return 0XFF;		//强制退出
	}
 	res=exf_fdcopy(fcpymsg,(uint8_t*)APP_SYSTEM_SYSICOPATH,(uint8_t*)APP_SYSTEM_DSTPATH,&totsize,&cpdsize,1);
	if(res)
	{

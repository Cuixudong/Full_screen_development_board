...
//标题表
const uint8_t* appplay_caption_tbl[GUI_LANGUAGE_NUM]=
{
"应用中心","應用中心","Applications"
};
//应用程序名列表
//这里列表为16个,名字长度不要超过8个字节(4个汉字)
uint8_t*const appplay_appname_tbl[GUI_LANGUAGE_NUM][16]=
{
{
	"红外遥控","应用程序","应用程序","应用程序",
	"应用程序","应用程序","应用程序","应用程序",
...
//appplay的图标路径表
uint8_t*const appplay_icospath_tbl[16]=
{
...
uint8_t appplay_load_ui(m_app_dev *appdev) 
{
	...
	if(twin)
	{
		...
		gui_fill_rectangle(0,0,lcddev.width,lcddev.height,APPPLAY_EX_BACKCOLOR);//填充背景色
 		...
 		gui_show_strmid(0,0,lcddev.width,APP_TEST_TOPBAR_HEIGHT,WHITE,16,(uint8_t*)appplay_caption_tbl[gui_phy.language]);//显示标题  
		window_draw(twin);
 		for(i=0;i<16;i++)
		{
			...
			appdev->icos[i].path=(uint8_t*)appplay_icospath_tbl[i];

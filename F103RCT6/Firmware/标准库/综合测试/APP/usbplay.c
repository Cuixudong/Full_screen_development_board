...
extern uint8_t*const nes_remindmsg_tbl[5][GUI_LANGUAGE_NUM];//在nesplay.c里面定义
//USB连接  				 
...
{			  
	...
	uint8_t i=0,err_back=0;
 	uint16_t werr=0,rerr=0;

   	POINT_COLOR=BLUE;  
  	BACK_COLOR=LGRAY;  
	LCD_Clear(BACK_COLOR);		//清屏
	app_filebrower((uint8_t*)APP_MFUNS_CAPTION_TBL[2][gui_phy.language],0X05);//显示标题  
	...
	if(Data_Buffer!=NULL&&Bulk_Data_Buff!=NULL)
	{
		...
		{  		
			Max_Lun=1;						//有2个磁盘 
			Mass_Memory_Size[1]=(long long)SD_GetSectorCount()*512;//得到SD卡容量，当SD卡容量超过4G的时候,需要用到两个uint32_t来表示

...
//画笔设置
const uint8_t* paint_penset_tbl[GUI_LANGUAGE_NUM]=
{"画笔设置","畫筆設置","PEN SET"};
//模式选择
const uint8_t* paint_mode_tbl[GUI_LANGUAGE_NUM][2]=
{
    {"新建画板","打开位图",},
    {"新建畫板","打開位圖",},
...
//画笔设置选择
const uint8_t* pen_set_tbl[GUI_LANGUAGE_NUM][2]=
{
    {"画笔颜色设置","画笔尺寸设置",},
    {"畫筆顏色設置","畫筆尺寸設置",},
...
//彩条颜色定义
const uint16_t colorbar_tbl[20]= {0XF800,0XFFE0,0X07E0,0X07FF,0X001F,0XF81F,0XFFF0,0X07F0,0X87FF,0X841F,
...
uint8_t paint_play(void)
{
    ...
    if(res==0)//确认键按下了,同时按键正常创建
    {
        ...
        } else	//选择一个BMP文件打开
        {
            ...
            else gui_memset((uint8_t*)paintinfo.lfname,0,paintinfo.lfsize);
            ...
            while(rval==0)
            {
                ...
                if(flistbox->dbclick==0X81)//双击文件了
                {
                    ...
                    if(rval)break;//打开成功
                    fn=(uint8_t*)(*paintinfo.lfname?paintinfo.lfname:paintinfo.fname);
    ...
    if(editmask)//有编辑过,需要保存
    {
        ...
        res=window_msg_box((lcddev.width-180)/2,(lcddev.height-80)/2,180,80,"",(uint8_t*)APP_SAVE_CAPTION_TBL[gui_phy.language],12,0,0X03,0);
